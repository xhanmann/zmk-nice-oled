...
// const lv_img_dsc_t *luna_imgs_bark_90[] = {&dog_sit1, &dog_sit2,  &dog_walk1, &dog_walk2,
// &dog_run1, &dog_run2, &dog_bark1, &dog_bark2, &dog_sneak1, &dog_sneak2};
...
static void set_hid_indicators(lv_obj_t *label, struct hid_indicators_state state) {
    ...
    if (state.hid_indicators & (LED_CLCK | LED_NLCK | LED_SLCK)) {
        ...
        if (!hid_anim) { // Si no existe aún, creamos la animación
            ...
            lv_animimg_set_duration(hid_anim,
                                    CONFIG_NICE_OLED_WIDGET_HID_INDICATORS_LUNA_ANIMATION_MS);
    ...
    // 1. &custom_font_outline = white 2. &custom_font_shadow = black 3. &custom_font_44 =
    // white &custom_font_44, &pixel_operator_mono_8, &pixel_operator_mono_12,
    // &pixel_operator_mono
    // lv_obj_set_style_text_font(label, &custom_font_22, LV_PART_MAIN | LV_STATE_DEFAULT);
    // params: obj, align, y, x, default: 33, 11
    // lv_obj_align(label, LV_ALIGN_OUT_TOP_LEFT, 30, -4); // big 44 words
    // lv_obj_align(label, LV_ALIGN_OUT_TOP_LEFT, 30, 6); // big 22 words

    // WORK FINE!!!
    // lv_obj_set_style_text_font(label, &custom_font_44, LV_PART_MAIN | LV_STATE_DEFAULT);
    // lv_label_set_text(label, text);
    // lv_obj_align(label, LV_ALIGN_OUT_TOP_LEFT, 3, -4); // big point
}
